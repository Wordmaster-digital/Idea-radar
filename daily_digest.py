...
FEEDS = [
    ...
    ("spring", "혁신",  "https://www.springwise.com/feed/",               "산업"),
...
def to_blocks(items, header):
    ...
    for it in items:
        ...
        line = f"{mark} **[{it['source']}]**{cat} [{it['title'][:100]}]({strip_utm(it['url'])}){sc}"
        if it.get("ko"):
            line += f"\n　↳ {it['ko']}"
        elif it["extra"]:
            line += f"\n　↳ {it['extra'][:120]}"
...
def main():
    ...
    p.add_argument("--sources", default="hn,gh,lob,dev,ph,yanko,dbm,d77,hack,atlas,food,retail,cool,spring,biz,tcs,platum,vsq,outs",
                   help="쉼표로 구분. 생략 시 전체")
    args = p.parse_args()
    ...
    def rank(x):
        if x["score"]:
            return x["score"] * WEIGHT.get(x["source"], 1)
        return NO_SCORE_RANK.get(x["source"], DEFAULT_RANK)
